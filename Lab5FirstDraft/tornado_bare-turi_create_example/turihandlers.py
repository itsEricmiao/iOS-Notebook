...
class UpdateUserChosenModelForDatasetId(BaseHandler):
    def get(self):
        ...
        model_type = self.get_argument("modelType")

        data = self.get_features_and_labels_as_SFrame(dsid)

        print("hey we're in the right one and the model_type is: ", model_type)
        # fit the model to the data
        acc = -1
        best_model = 'unknown'
        if len(data)>0:
            if (model_type == "BTC"):
                ...
                model = tc.nearest_neighbor_classifier.create(data,target='target',verbose=0)# training
            else:
                model = tc.classifier.create(data,target='target',verbose=0)# training
            yhat = model.predict(data)
            self.clf[dsid] = model
