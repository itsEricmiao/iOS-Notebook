...
let SERVER_URL = "http://10.8.103.153:8000" // change this for your server name!!!
...
class ViewController: UIViewController, URLSessionDelegate, UIPickerViewDelegate, UIPickerViewDataSource{
    ...
    // PickerView Reference: www.codewithchris.com/uipickerview-example/
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        switch pickerView {
            case mlModelPicker:
                mlModelLabel.text = pickerData[row]
            case mlModelPicker:
                mlModelLabel.text = pickerData[row]
            default:
                break
    ...
    @IBOutlet weak var mlModelPicker: UIPickerView!
    @IBOutlet weak var mlModelLabel: UILabel!
    ...
    override func viewDidLoad() {
        ...
        self.mlModelPicker.delegate = self
        self.mlModelPicker.dataSource = self
        pickerData = ["Model-1", "Model-2", "Model-3"]
    ...
    func getPrediction(_ array:[Double]){
        ...
        let postTask : URLSessionDataTask = self.session.dataTask(with: request,
                        ...
                        if(error != nil){
                            ...
                            print(labelResponse)
                            if(labelResponse as! String == "DNE"){
                                let alert = UIAlertController(title: "Model Doesn't Exist", message: "The model you are trying to use doesn't exist, you need to train it first", preferredStyle: .alert)
    ...
    @IBAction func makeModel(_ sender: AnyObject) {
        ...
        let query = "?dsid=\(self.dsid)&modelType=BTC"
