...
//@property (nonatomic) NSMutableArray *redArr;
//@property (nonatomic) NSMutableArray *greenArr;
//@property (nonatomic) NSMutableArray *blueArr;
...
//NSMutableArray *redArr = [[NSMutableArray alloc] initWithCapacity:100];
//NSMutableArray *blueArr = [[NSMutableArray alloc] initWithCapacity:100];
//NSMutableArray *greenArr = [[NSMutableArray alloc] initWithCapacity:100];
int redArr[100];
int blueArr[100];
int greenArr[100];
...
-(bool)processFinger{
    ...
    sprintf(text,"Place Finger Flush on Camera", avgPixelIntensity.val[0],avgPixelIntensity.val[1],avgPixelIntensity.val[2]);
    cv::putText(_image, text, cv::Point(50, 50), FONT_HERSHEY_PLAIN, 0.75, Scalar::all(255), 1, 2);
    sprintf(text,"or Flash Will Occur", avgPixelIntensity.val[0],avgPixelIntensity.val[1],avgPixelIntensity.val[2]);
    ...
    cv::putText(_image, text, cv::Point(50, 90), FONT_HERSHEY_PLAIN, 0.75, Scalar::all(255), 1, 2);
    
    if(avgPixelIntensity.val[0] + avgPixelIntensity.val[1] + avgPixelIntensity.val[2] <=200) {
        redArr[avgArrIdx] = avgPixelIntensity.val[0];
        greenArr[avgArrIdx] = avgPixelIntensity.val[1];
        blueArr[avgArrIdx] = avgPixelIntensity.val[2];
        std::cout << avgArrIdx << std::endl;
        if (avgArrIdx >= 100) {
            sprintf(text,"100 Frames Reached", avgPixelIntensity.val[0],avgPixelIntensity.val[1],avgPixelIntensity.val[2]);
            cv::putText(_image, text, cv::Point(50, 110), FONT_HERSHEY_PLAIN, 0.75, Scalar::all(255), 1, 2);
        } else {
            avgArrIdx++;
        }
        lastBool = true;
    } else if(lastBool && avgPixelIntensity.val[0] + avgPixelIntensity.val[1] + avgPixelIntensity.val[2] <= 300) {
        redArr[avgArrIdx] = avgPixelIntensity.val[0];
        greenArr[avgArrIdx] = avgPixelIntensity.val[1];
        blueArr[avgArrIdx] = avgPixelIntensity.val[2];
        std::cout << avgArrIdx << std::endl;
    ...
    else if (avgPixelIntensity.val[0] + avgPixelIntensity.val[1] + avgPixelIntensity.val[2] >=400) {
        lastBool = false;
    }
